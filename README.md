...
Ce projet a demandé une charge de travail plus importante que les anciens projets. De part le déploiement de plusieurs bases de données formant une structure complexe ou encore la réalisation du générateur insérant des données dans toutes les bases ainsi que la définition de nombreux indicateurs pour créer une solution BI fonctionnelle, le temps accordé à ce projet n'était pas suffisant.  
![](https://github.com/Heavyshield/PlasticBox/blob/master/annexe/CF2.png)  
Ce projet était très interessant: il touchait à plusieurs domaines de l'informatique, nous permettait d'appliquer des mathématiques et de nous plonger dans les "Big Data". Cependant, le scope du projet s'est affiné en cours de projet, donnant une charge de travail supplémentaire et obligeant notre équipe à être très flexible.  
